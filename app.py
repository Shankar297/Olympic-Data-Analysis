...
if user_menu == 'Overall Analysis':
    ...
    fig.update_xaxes(tickfont_family='Courier New',title_font=dict(size=20,family='Courier'))
    fig.update_yaxes(tickfont_family='Courier New',title_font=dict(size=20,family='Courier'))
    fig.update_layout(font=dict(family='Courier',size=17))
    st.plotly_chart(fig)
    ...
    st.title('Athlets Over Time')
    event_over_time = helper.data_over_time(df,'Name')
    fig = px.line(event_over_time,x="Edition",y="Name")
    fig.update_xaxes(tickfont_family='Courier New',title_font=dict(size=20,family='Courier'))
    fig.update_yaxes(tickfont_family='Courier New',title_font=dict(size=20,family='Courier'))
    fig.update_layout(font=dict(family='Courier',size=17))
    st.plotly_chart(fig)
    ...
    ax = sns.heatmap(x.pivot_table(index='Sport',columns='Year',values='Event',aggfunc='count').fillna(0).astype('int'),annot = True,annot_kws={'size':18})
...
if user_menu == 'Athlete wise Analysis':
    ...
    name_list.sort()
    selected_name = st.selectbox("Select a Athlete Name",name_list)
    new_df = helper.athlete_wise(df,selected_name)
